...
#else
		case NJ_ISSEN:
			wd->damage = 80 * sstatus->str + sstatus->hp * 8 * skill_lv / 100;
			wd->damage2 = 0;
			break;
		case LK_SPIRALPIERCE:
		case ML_SPIRALPIERCE:
		...
		case RK_DRAGONBREATH_WATER:
			{
				...
				if(sd)
					damagevalue = damagevalue * (50 + 5 * (pc_checkskill(sd,RK_DRAGONTRAINING) - 1)) / 100;
		...
		switch(skill_id) {
			...
			case RK_DRAGONBREATH_WATER:
				if(wd.flag&BF_LONG) { //Add check here, because we want to apply the same behavior in pre-renewal [exneval]
					wd.damage = wd.damage * (50 + sd->bonus.long_attack_atk_rate) / 100;
					if(is_attack_left_handed(src, skill_id))
						wd.damage2 = wd.damage2 * (50 + sd->bonus.long_attack_atk_rate) / 100;
				}
				break;
			default:
			...
			switch(skill_id) {
				...
				case MG_THUNDERSTORM:
					if(sc->data[SC_GUST_OPTION])
						ad.damage += (10 + sstatus->int_ / 4) + max(sstatus->dex - 10, 0) / 30;
					break;
				case MG_FIREBOLT:
				case MG_FIREWALL:
